    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for row in factor.getAllPossibleAssignmentDicts():
                ...
                if getHouseVal(obsPos[0], obsPos[1], gameState) == row[FOOD_HOUSE_VAR]:
                    redWall  = PROB_FOOD_RED
                    blueWall = 1 - redWall
                    noWall   = 1 - redWall - blueWall

                elif getHouseVal(obsPos[0], obsPos[1], gameState) == row[GHOST_HOUSE_VAR]:
