...
def normalize(factor):
    ...
    for var in factor.unconditionedVariables():
        if len(variableDomainsDict[var]) == 1:
            condVars.add(var)
            unCondVars.remove(var)

    newFactor = Factor(unCondVars, condVars, variableDomainsDict)
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        normalizedProb = factor.getProbability(assignment) / probSum
        newFactor.setProbability(assignment, normalizedProb)
